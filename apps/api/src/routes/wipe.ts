...
const wipeBody = {
  type: 'object',
  required: ['wipeType', 'keepPlayerData', 'keepAccessLists', 'createBackup', 'serverWillRestart'],
  properties: {
    wipeType: { type: 'string', enum: ['FULL', 'MAP_ONLY', 'MAP_PLAYERS', 'CUSTOM'] },
    customFiles: { type: 'array', items: { type: 'string' } },
    keepPlayerData: { type: 'boolean' },
    keepAccessLists: { type: 'boolean' },
    createBackup: { type: 'boolean' },
    serverWillRestart: { type: 'boolean' },
    notes: { type: 'string' },
  },
} as const
