...
describe('Users routes', () => {
  ...
  describe('POST /api/users', () => {
    ...
    afterAll(async () => {
      ...
      await ctx.prisma.session.deleteMany({
        where: { user: { email: { in: ['newviewer@test.com', 'newadmin@test.com'] } } },
      })
      await ctx.prisma.account.deleteMany({
        where: { user: { email: { in: ['newviewer@test.com', 'newadmin@test.com'] } } },
      })
      await ctx.prisma.user.deleteMany({
        where: { email: { in: ['newviewer@test.com', 'newadmin@test.com'] } },
      })
    })
  })
  ...
  describe('PUT /api/users/:id/role', () => {
    it("OWNER can change another user's role", async () => {
      const users: { id: string; role: string }[] = JSON.parse(
        (await ctx.owner.get('/api/users')).body
      )
