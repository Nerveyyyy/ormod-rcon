  ...
  return (
    <div className="main fadein">
      <PageHeader
        ...
        actions={<button className="btn btn-primary btn-sm" onClick={() => setShowAdd(true)}>+ Create User</button>}
      />

      {/* ── Create User Modal ──────────────────────────────── */}
      {showAdd && (
        <div className="overlay" onClick={() => setShowAdd(false)}>
          <div className="modal fadein" onClick={e => e.stopPropagation()} style={{ maxWidth: '480px' }}>
            ...
            <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div className="setting-row" style={{ padding: 0 }}>
                ...
                <input className="text-input" value={name} onChange={e => setName(e.target.value)} placeholder="John Doe" />
              </div>
              <div className="setting-row" style={{ padding: 0 }}>
                <div className="setting-info">
                  <div className="setting-name">Email</div>
                </div>
                <input className="text-input" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="user@example.com" />
              </div>
              <div className="setting-row" style={{ padding: 0 }}>
                <div className="setting-info">
              ...
              <div className="setting-row" style={{ padding: 0 }}>
                ...
                <select className="sel-input" value={role} onChange={e => setRole(e.target.value as 'ADMIN' | 'VIEWER')}>
      ...
      {deleteTarget && (
        ...
        >
          <div style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: '1.6' }}>
            This will permanently delete <strong style={{ color: 'var(--orange)' }}>{deleteTarget.name}</strong> ({deleteTarget.email}).
            All their sessions will be invalidated.
          </div>
        </ConfirmDialog>
      )}
            ...
            <tbody>
              {users.map(u => {
                ...
                const roleCls =
                  ...
                  u.role === 'ADMIN' ? 'role-admin' : 'role-viewer';
                return (
                  <tr key={u.id}>
                    <td className="bright">
                      {u.name}
                      {self && <span style={{ fontSize: '10px', color: 'var(--dim)', marginLeft: '6px' }}>(you)</span>}
                    </td>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      {!self && (
                        <div className="btn-group" style={{ justifyContent: 'flex-end' }}>
                          ...
                          <button className="btn btn-danger btn-xs" onClick={() => setDeleteTarget(u)}>Delete</button>
                        </div>
                      )}
                    </td>
                  </tr>
