...
const wipeTypes = [
  {
    id: 'MAP_ONLY', label: 'Map Wipe', color: 'btn-primary', icon: '◫',
  ...
  {
    id: 'MAP_PLAYERS', label: 'Map + Players', color: 'btn-ghost', icon: '◫',
  ...
  {
    id: 'FULL', label: 'Full Wipe', color: 'btn-danger', icon: '⊠',
...
export default function WipeManager() {
  ...
  const loadHistory = () => {
    ...
    api.get<WipeLog[]>(`/servers/${activeServer.id}/wipes`)
      .then(setHistory)
      .catch(console.error);
  };

  useEffect(() => { loadHistory(); }, [activeServer?.id, tab]);

  const chosen = wipeTypes.find(w => w.id === pendingType);

  const openConfirm  = (id: string) => { setPendingType(id); setShowConfirm(true); };
  const closeConfirm = () => { setShowConfirm(false); setPendingType(null); };
      ...
      <PageHeader
        ...
        actions={
          <div className="btn-group">
            ...
            <button onClick={() => setTab('history')} className={`btn btn-sm ${tab === 'history' ? 'btn-primary' : 'btn-ghost'}`}>History ({history.length})</button>
          </div>
        }
      />

      <div className="warn-banner">
        ⚠ Wipes are <strong style={{ margin: '0 3px' }}>irreversible</strong>. The server will be stopped automatically before any wipe executes.
      </div>
          ...
          {wipeTypes.map(w => (
            <div key={w.id} className="card">
              <div className="card-header">
                <span className="card-title">{w.icon} {w.label}</span>
              </div>
              <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <p style={{ fontSize: '12px', color: 'var(--muted)', lineHeight: '1.6' }}>{w.desc}</p>
                <div>
                  <div style={{ fontSize: '10px', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Deletes</div>
                  {w.deletes.slice(0, 4).map(f => (
                    <div key={f} style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--red)', padding: '1px 0' }}>✕ {f}</div>
                  ))}
                  {w.deletes.length > 4 && (
                    <div style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: 'var(--dim)' }}>+ {w.deletes.length - 4} more</div>
                  )}
                </div>
                <button className={`btn ${w.color} btn-sm`} style={{ marginTop: 'auto' }}
                  onClick={() => openConfirm(w.id)} disabled={wiping || !activeServer?.id}>
                  Execute {w.label}
                </button>
              </div>
            </div>
      ...
      {tab === 'history' && (
        <div className="card">
          ...
          <table className="data-table">
            <thead><tr><th>Type</th><th>Triggered By</th><th>Date</th><th>Notes</th><th>Backup</th></tr></thead>
            <tbody>
              {history.length === 0 && (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '24px', color: 'var(--dim)' }}>No wipe history.</td></tr>
              )}
              {history.map(w => (
                <tr key={w.id}>
                  <td><span className="pill pill-orange">{w.wipeType}</span></td>
                  <td className="bright">{w.triggeredBy}</td>
                  <td className="mono" style={{ color: 'var(--dim)' }}>{new Date(w.createdAt).toLocaleString()}</td>
                  <td>{w.notes ?? '—'}</td>
                  <td>{w.backupPath ? <span className="pill pill-green">✓ Backup</span> : <span className="pill pill-muted">None</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showConfirm && chosen && (
        ...
        >
          <div style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: '1.6' }}>
            ...
            <strong style={{ color: 'var(--orange)' }}> {activeServer?.name ?? 'this server'}</strong>. The server will be stopped first.
          </div>
          ...
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Create backup before wiping</div></div>
            <div className={`toggle ${createBackup ? 'on' : ''}`} onClick={() => setCreateBackup(p => !p)} />
          </div>
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Restart server after wipe</div></div>
