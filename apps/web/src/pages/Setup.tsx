    ...
    <div style={{
      ...
      <div className="card fadein" style={{ width: '420px' }}>
        ...
        <div className="card-body" style={{ marginBottom: '8px' }}>
          <p style={{ fontFamily: 'var(--mono)', fontSize: '11px', color: 'var(--muted)', lineHeight: '1.7', margin: 0 }}>
            No users exist yet. Create the <strong style={{ color: 'var(--orange)' }}>OWNER</strong> account —
            this account has full administrative access and can create additional users.
          </p>
        </div>
        <form className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }} onSubmit={submit}>
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Display Name</div></div>
            <input className="text-input" required value={name}
              onChange={e => setName(e.target.value)} placeholder="Server Admin" />
          </div>
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Email</div></div>
            <input className="text-input" type="email" autoComplete="email" required
              value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@example.com" />
          </div>
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Password</div></div>
            <input className="text-input" type="password" autoComplete="new-password" required
              value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 8 characters" />
          </div>
          <div className="setting-row" style={{ padding: 0 }}>
            <div className="setting-info"><div className="setting-name">Confirm Password</div></div>
            <input className="text-input" type="password" autoComplete="new-password" required
